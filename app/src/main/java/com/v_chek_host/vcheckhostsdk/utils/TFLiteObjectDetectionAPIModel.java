...
public class TFLiteObjectDetectionAPIModel implements Detector {
  ...
  private static MappedByteBuffer loadMappedFile(@NonNull Context context, @NonNull String filePath) throws IOException {
...
*/
  ...
  public static Detector create(
   ...
   //MappedByteBuffer modelFile = loadModelFile(context.getAssets(), modelFilename);
    MappedByteBuffer modelFile = loadMappedFile(context,modelFilename);
    //MetadataExtractor metadata = new MetadataExtractor(modelFile);
/*
    try (BufferedReader br =
        new BufferedReader(
            new InputStreamReader(
                    context.getAssets().open(labelFilename)))) {
...
*/
  ...
  @Override
  public List<Recognition> recognizeImage(final Bitmap bitmap,int size) {
    ...
    boolean nameBoardFlag=false;
    for (int i = 0; i < numDetectionsOutput; ++i) {
      final RectF detection =
          new RectF(
              outputLocations[0][i][1] * size,
              outputLocations[0][i][0] * size,
              outputLocations[0][i][3] * size,
              outputLocations[0][i][2] * size);

      Recognition recognition = new
              Recognition("" + i, labels.get((int) outputClasses[0][i]), outputScores[0][i], detection);
      int val = (int)(recognition.getConfidence()*100);
      if(recognition.getTitle().equalsIgnoreCase("Negative")){
        //Skip the Negative
      }else{
        if (val<50){

        }else {
          /*if (recognition.getTitle().equalsIgnoreCase("GearSet")){
            if (!shifterFlag){
              shifterFlag=true;
              recognitions.add(recognition);
            }
          }*/
          if (recognition.getTitle().equalsIgnoreCase("GearShifter")) {
            if(!shifterFlag){
              shifterFlag=true;
              recognitions.add(recognition);
            }
          } else if(recognition.getTitle().equalsIgnoreCase("GearNamesBoard")){
            if(!nameBoardFlag){
              nameBoardFlag=true;
              recognitions.add(recognition);
            }
  ...
  public static Bitmap cropedImage(Bitmap picture, int width, int height, int imageCropX, int imageCropY, int imageCropWidth, int imageCropHeight) {
    ...
    scaledBitmap = Bitmap.createScaledBitmap(picture, width, height, true);
    cropedBitmap = Bitmap.createBitmap(scaledBitmap, imageCropX, imageCropY, imageCropWidth, imageCropHeight);
    return cropedBitmap;
  }
